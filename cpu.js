...
class CPU {
  ...
  execute(instruction) {
    switch (instruction) {
      ...
      case instructions.SHOVE_LTT_REG: {
      ...
      case instructions.SHOVE_REG_REG: {
      ...
      case instructions.SHOVE_REG_MEM: {
        const registerFrom = (this.fetch() % this.registerNames.length) * 2;
      ...
      case instructions.SHOVE_MEM_REG: {
        const address = this.fetch16();
        const registerTo = (this.fetch() % this.registerNames.length) * 2;
      ...
      case instructions.SMUSH_REG_REG: {
        const R1 = this.fetch();
        const R2 = this.fetch();
